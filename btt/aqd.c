...
void aqd_issue_size(void *info, double ts, size_t size)
{
	if (info) {
		struct aqd_info *ap = info;
		fprintf(ap->fp, "%lf %d %zu\n%lf %d %zu\n", ts, ap->na, size, ts, ap->na + 1, size);
		ap->na += 1;
	}
}
...
void aqd_complete_size(void *info, double ts, size_t size)
{
	if (info) {
		struct aqd_info *ap = info;
